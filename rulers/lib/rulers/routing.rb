# this class handles declaration of routes and finding a rack app for a route
class RouteObject
  def initialize
    @rules = []
  end

  # for building routes at app start
  def match(url, *args)
    options = {}
    options = args.pop() if args[-1].is_a?(Hash) # if last argument is hash, this is an options object
    options[:default] ||= {}
  ...
  # for matching request url to route
  def check_url(url)
    @rules.each do |r|
...
module Rulers
  class Application
    def route(&block)
      ...
      @route_obj.instance_eval(&block) # instance_eval will call a block with route_obj set to self
    end
